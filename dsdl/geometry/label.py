    ...
    def visualize(self, image, draw_obj, palette, **kwargs):
        ...
        for label_obj in self.label_list:
            ...
            if "bbox" in kwargs:
                ...
                coords = y_offset + np.array([[item.xyxy[0], item.xyxy[1] + 0.2 * label_size[1]] for item in kwargs["bbox"].values()])
            ...
            for coord in coords:
                draw_obj.rectangle([tuple(coord), tuple(coord + label_size)], fill=color)
                draw_obj.text(tuple(coord), category_name, fill=(255, 255, 255), font=font)
